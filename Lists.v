...
Fixpoint subset (s1:bag) (s2:bag) : bool :=
  match s1 with
  | nil => true
  | h1 :: t1 => match leb (count h1 s1) (count h1 s2) with
    | true => subset t1 s2
    | false => false
    end
  end.
...
Proof.
  intro l1. intro l2. intro l3. intro l4.
  rewrite -> app_assoc.
  rewrite -> app_assoc.
  reflexivity.
Qed.
...
Proof.
  induction l as [| n l' Hl'].
  - reflexivity.
  - simpl. rewrite -> self_eq. rewrite <- Hl'. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - reflexivity.
  - simpl. destruct n as [| n'].
    + simpl. rewrite -> plus_1_r. rewrite -> leb_n_Sn. reflexivity.
